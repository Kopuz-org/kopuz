...
impl JoinEvidence {
    fn observe(&mut self, credits: &[String]) {
        if let [whole] = credits {
            *self
                .whole_credit
                .entry(utils::artist::name_key(whole))
                .or_default() += 1;
        }
    }
    ...
    fn tracks_credited_solely_to(&self, name: &str) -> usize {
        self.whole_credit
            .get(&utils::artist::name_key(name))
            .copied()
            .unwrap_or(0)
    }
    ...
    fn resolve(&self, credit: &str) -> Option<Vec<String>> {
        let pieces = utils::artist::join_candidates(credit)?;
...
fn credits_from_strings(artist: &str, stored: &[String]) -> Vec<String> {
    ...
    let derived = utils::artist::split_credit(artist);
    ...
    let says_nothing = derived.len() == 1
        && derived[0] == artist
        && utils::artist::join_candidates(artist).is_none();
    if derived.is_empty() || (says_nothing && stored.len() > 1) {
        utils::artist::credited(artist, stored)
    } else {
        derived
    }
}
...
fn push_unique(out: &mut Vec<String>, name: String) {
    let key = utils::artist::name_key(&name);
    if key.is_empty() {
        return;
    }
    if !out.iter().any(|seen| utils::artist::name_key(seen) == key) {
        out.push(name);
    }
}
