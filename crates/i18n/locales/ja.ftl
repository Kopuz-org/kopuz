language-name = 日本語
is_rtl = false

settings = 設定
settings_layout = 設定ナビゲーション
settings_layout_cd = CD
settings_layout_topbar = トップバー
theme_editor = テーマエディター
appearance = 外観
language = 言語
library = ライブラリ
playlists = プレイリスト
album = アルバム
artist = アーティスト
home = ホーム
search = 検索
favorites = お気に入り
radio = ラジオ
discover = 見つける
logs = ログ
activity = アクティビティ
general = 一般
local_libraries = ローカルライブラリ
active_local_library = 使用中
switch_to_local_library = 切り替え
add_local_library = ローカルライブラリを追加
local_library_name = ライブラリ名
local_library_name_required = ライブラリ名は必須です
local_library_folder_required = 1つ以上のフォルダを選択してください
service = サービス: { $name }
connected = ● 接続済み
disconnected = ● 未接続
connecting = ● 接続中…
reconnect = 再接続
remove = 削除
add = 追加
add_folder = フォルダを追加
no_music_folders = 音楽フォルダが設定されていません
remote_music_folders = 音楽フォルダ
parent_folder = 上へ
use_this_folder = このフォルダを追加
no_subfolders = サブフォルダがありません
loading_folders = フォルダを読み込み中…
folder_browse_failed = フォルダを一覧できませんでした: { $error }
add_server = サーバーを追加
connect_to_lastfm = Last.fm に接続
connect_to_librefm = Libre.fm に接続
lastfm_connected = Last.fm 接続済み
librefm_connected = Libre.fm 接続済み
server_details = サーバーの詳細
server_name = サーバー名
server_url = サーバーURL
choose_service = サービスを選択
cancel = キャンセル
login = ログイン
logging_in = ログイン中...
login_to_service = { $service } にログイン
username = ユーザー名
password = パスワード
player_settings = プレイヤー設定
discord_presence = Discordプレゼンス
connectivity = 接続
discord_presence_paused = 一時停止中のDiscordステータスを表示
discord_presence_source = Discordステータスにメディアソースを表示
reduce_animations = アニメーションを減らす
fullscreen_use_player_bar = 全画面表示でプレーヤーバーを操作に使う
cover_art_background = カバーアートを背景に表示
cover_art_darkening = カバーアートの暗さ
cover_art_blur = カバーアートのぼかし
custom_background = カスタム背景
choose_image = 画像を選択
custom_font = カスタムフォント
choose_font = フォントを選択
live_theme = Matugen / Pywal（ライブ）
live_theme_file = パレットファイル
choose_palette = パレットを選択
jump_to_current_song = 再生中の曲へ移動
hide_side_panel = サイドパネルを隠す
show_side_panel = サイドパネルを表示
show_source_toggle = ソース切替を表示
show_row_images = 行に画像を表示
titlebar_mode = タイトルバー
titlebar_custom = カスタム（アプリ描画）
titlebar_system = システム装飾
titlebar_off = タイトルバーなし
volume = 音量
local = ローカル
source = Source
sources = Sources
manage_sources = Manage sources
source_on_this_device = On this device
server = サーバー
youtube_downloads = ダウンロード
youtube_download_title = YouTube ダウンロード
youtube_download_subtitle = YouTube Music を名前で検索するか、リンクを貼り付けて Kopuz でダウンロード
youtube_download_options = オプション
youtube_download_search_placeholder = 曲名、アーティスト、アルバム、またはリンクを貼り付け…
youtube_download_search = 検索
youtube_download_download_link = リンクをダウンロード
youtube_download_resolving_link = リンクを読み込み中…
youtube_download_link_detected = リンクを検出しました。その曲をそのままダウンロードします
youtube_download_error_link = リンクを読み込めませんでした: { $error }
youtube_download_searching = 検索中…
youtube_download_results = 検索結果
youtube_download_download = ダウンロード
youtube_download_output_dir_placeholder = 保存先フォルダ（既定はミュージックフォルダ）
youtube_download_pick_folder = フォルダを選択
youtube_download_clear_history = 履歴を消去
youtube_download_empty_state = 曲を検索するか、YouTube のリンクを貼り付けてください
youtube_download_no_results = 曲が見つかりません
youtube_download_jobs = ダウンロード
youtube_download_file_options = ファイルオプション
youtube_download_embed_metadata = メタデータを埋め込む
youtube_download_embed_metadata_desc = タイトル、アーティスト、アルバム、取得元 URL を音声ファイルに保存します
youtube_download_embed_thumbnail = カバーアートを埋め込む
youtube_download_embed_thumbnail_desc = 対応する形式なら埋め込み、そうでなければ JPG を隣に保存します
youtube_download_write_thumbnail = カバーを別途保存
youtube_download_write_thumbnail_desc = ダウンロードした音声の隣に JPG を書き出します
youtube_download_organize_album = アルバムごとに整理
youtube_download_organize_album_desc = アルバム名のフォルダにダウンロードを入れます
youtube_download_overwrite = 既存のファイルを上書き
youtube_download_overwrite_desc = 同じ出力名のファイルを置き換えます
youtube_download_ffmpeg_hint = オリジナル品質のダウンロードに外部ツールは不要です。ffmpeg は変換、メタデータ、カバーの埋め込みにのみ必要です。
youtube_download_format_original = オリジナル品質
youtube_download_format_mp3 = MP3
youtube_download_format_flac = FLAC
youtube_download_format_opus = OPUS
youtube_download_format_wav = WAV
youtube_download_status_resolving = YouTube のストリームを解決中…
youtube_download_status_downloading_eta = { $percent }%  { $speed }  残り { $eta }
youtube_download_status_downloading = { $percent }%
youtube_download_status_processing = 処理中…
youtube_download_status_completed = 完了
youtube_download_status_waiting = 待機中…
youtube_download_error_search = 検索に失敗しました: { $error }
youtube_download_error_ffmpeg_required = この形式または選択したファイルオプションには ffmpeg が必要です。ffmpeg をインストールするか、オリジナル品質を選んで埋め込みを無効にしてください。
youtube_download_error_duplicate_active = この曲はすでにダウンロード中です。
youtube_download_error_output_not_directory = 保存先のパスがフォルダではありません: { $path }
youtube_download_error_output_prepare = 保存先フォルダを準備できませんでした: { $error }
youtube_download_error_output_not_writable = 保存先フォルダに書き込めません: { $path }

# UI Actions & Buttons
add_to_favorites = お気に入りに追加
remove_from_favorites = お気に入りから削除
add_to_queue = キューに追加
play_next = 次に再生
add_to_playlist = プレイリストに追加
remove_from_playlist = プレイリストから削除
start_radio = ラジオを開始
radio_starting = ラジオを開始しています…
radio_empty = 類似する曲が見つかりません
radio_failed = ラジオを開始できませんでした
share_musicbrainz = 共有 (MusicBrainz)
share_link_copied = MusicBrainzのリンクをコピーしました
delete = 削除
delete_song = 曲を削除
delete_album = アルバムを削除
delete_playlist = プレイリストを削除
delete_folder = フォルダーを削除
rename_playlist = プレイリスト名を変更
rename_folder = フォルダー名を変更
add_all_to_queue = すべてをキューに追加
add_all_to_playlist = すべてをプレイリストに追加
select_all = すべて選択
select_all_tracks = すべてのトラックを選択
remove_from_cache = キャッシュから削除
create = 作成
save = 保存
enabled = 有効
disabled = 無効
play = 再生
shuffle = シャッフル

# Navigation & Headers
tracks = トラック
albums = アルバム
artists = アーティスト
all_albums = すべてのアルバム
your_library = あなたのライブラリ
listening_logs = 再生ログ
browse_genres = ジャンルを見る
top_artists = 人気アーティスト
new_releases = 新着

# Navigation Buttons
up_next = 次の曲
lyrics = 歌詞

# Player/Media
loading_lyrics = 歌詞を読み込み中...
lyrics_not_found = 歌詞が見つかりません
instrumental_break = 間奏
no_previous_songs = 以前の曲はありません
playlist_track_count = { $count } トラック
music_playlist_count = 音楽 • { $count } トラック
showcase_song_count = { $count } 曲

# Modal Titles
add_playlist = プレイリストを追加
create_new_playlist = 新しいプレイリストを作成
playlist_name_placeholder = プレイリスト名
playlist_name_input = プレイリスト名
add_media_server = メディアサーバーを追加
media_server = メディアサーバー
media_servers = メディアサーバー
saved_servers = 保存されたサーバー
switch_to_server = 切り替え
active_server = アクティブ
no_saved_servers = 保存されたサーバーはありません
jellyfin = Jellyfin
subsonic = Subsonic
custom_manual = カスタム（手動API）
server_name_optional = サーバー名（任意）
server_url_placeholder = http://localhost:8096
nextcloud_url_placeholder = https://cloud.example.com
change = 変更
heart_track_to_add = 再生中の曲をハートしてここに追加します。
heart_track_to_add_server = 再生中の曲をハートしてここに追加し、サーバーに同期します。

# Search & Placeholders
search_placeholder = アーティスト、アルバム、曲を検索...
quick_search_placeholder = 入力して検索、Enterで再生…
no_results_found = "{ $query }" の結果が見つかりません
listenbrainz_token_placeholder = ListenBrainzトークンを入力
lastfm_api_key_placeholder = Last.fm の API キーを入力
lastfm_api_secret_placeholder = Last.fm の API シークレットを入力

# Empty States
album_not_found = アルバムが見つかりません
playlist_not_found = プレイリストが見つかりません
no_playlists_found = プレイリストが見つかりません
no_playlists_yet = プレイリストがまだありません。ライブラリから曲を追加してください！
add_music_to_get_started = 開始するには音楽を追加してください

# Data Labels
title = タイトル
time = 時間
genre = ジャンル
plays = 再生回数
track_count = { $count } トラック
track_count_singular = 1トラック
songs = 曲
min = 分

# Error Messages
invalid_server_url = 無効なサーバーURL
server_name_required = サーバー名は必須です
username_and_password_required = ユーザー名とパスワードが必要です
login_failed = ログイン失敗: { $error }
error_server_not_configured = サーバーが設定されていないか、認証情報がありません
error_fetch_songs = アルバム '{ $album_id }' の曲を取得できませんでした: { $error }
unsupported_provider = { $service } は設定されていますが、このページはまだそのプロバイダーに対応していません
unsupported_provider_desc = { $service } のプロバイダー固有のブラウジングは新しいサーバー抽象化で追加される予定です

# Additional Keys
no_genres_found = ライブラリにジャンルが見つかりません。
jump_back_in = 続きから聴く
by_artist = アーティスト:
by_artist_full = { $artist } による
start_listening = 聴き始める
listen_now = 今すぐ聴く
no_albums_found = ライブラリにアルバムが見つかりません。
no_favorites = お気に入りがまだありません。
no_tracks_found = トラックが見つかりません。
unknown_artist = 不明なアーティスト
unknown_album = 不明なアルバム
unknown_title = 不明なタイトル
unknown_genre = 不明
most_played_local_tracks = 最も再生されたローカルトラック。
no_tracks_in_library = ライブラリにトラックが見つかりません。
no_songs_here = ここには曲がありません。
syncing_with_server = サーバーと同期中...
yt_syncing_progress = 同期中 — これまでに { $count } 曲を読み込みました
yt_synced_total = お気に入りの { $count } 曲を同期しました
refresh = 更新
most_played_tracks = 最も再生されたトラック。
no_more_songs = キューにこれ以上の曲はありません
server_playlist = サーバープレイリスト
folder_playlists = フォルダプレイリスト
folder_playlist = フォルダ
featured_album = 注目のアルバム
minimize = 最小化
maximize = 最大化
close = 閉じる
mini_player = ミニプレーヤー
restore_full_player = フルプレーヤーに戻す
shuffle_on = シャッフル: オン
shuffle_off = シャッフル: オフ
repeat_off = リピート: オフ
repeat_queue = リピート: キュー
repeat_track = リピート: トラック
rescan_library = ライブラリを再スキャン
refresh_music_library = 音楽ライブラリを更新
listenbrainz = ListenBrainz
lastfm = Last.fm
librefm = Libre.fm
enable_tracing = パフォーマンストレースを有効化
tracing_warning = オーバーヘッドが増え、トレースファイルが大きくなります。再起動後に有効になります。
export_logs = ログをエクスポート
open_logs_folder = ログフォルダーを開く
english = English
russian = Русский
toki_pona = toki pona
turkish = Türkçe
japanese = 日本語
arabic = العربية
album_art_gradient = アルバムアートグラデーション
default_theme = デフォルト
gruvbox_material = Gruvbox Material
gruvbox_classic = Gruvbox Classic
gruvbox_dark_soft = Gruvbox Dark Soft
dracula = Dracula
nord = Nord
catppuccin_mocha = Catppuccin Mocha
ef_night = Ef Night
ayu_dark = Ayu Dark
ayu_mirage = Ayu Mirage
vague = Vague
one_dark_pro = One Dark Pro
osmium = Osmium
kanagawa_dragon = Kanagawa Dragon
everforest = Everforest
rosepine = Rosé Pine
default_light = デフォルトライト
amoled_black = AMOLED ブラック
catppuccin_latte = Catppuccin Latte
rosepine_dawn = Rosé Pine Dawn
everforest_light = Everforest Light
ayu_light = Ayu Light
one_light = One Light
gruvbox_light_soft = Gruvbox Light Soft

# Theme Editor
new_theme = 新しいテーマ
theme_name = テーマ名
my_custom_theme = マイカスタムテーマ
theme_group_dynamic = ダイナミック
theme_group_dark = ダーク
theme_group_light = ライト
theme_group_custom = カスタム
colors = 色
preview = プレビュー
save_theme = テーマを保存
track_title = トラックタイトル

# Color Labels
bg = 背景
raised = 浮き上がった面
surface = 面
text = テキスト
text-muted = ミュートテキスト
accent = アクセント
accent-soft = ソフトアクセント
accent-alt = 代替アクセント
accent-deep = ディープアクセント
highlight = ハイライト
highlight-dark = ダークハイライト
progress = 進捗
danger = 危険
equalizer = イコライザー
eq_preset = プリセット
eq_preamp = プリアンプ
eq_preamp_desc = EQカーブ前のヘッドルーム
eq_graph_hint = バンドを調整するとプリセットはカスタムに切り替わります。
eq_preset_flat = フラット
eq_preset_bass_boost = 低音ブースト
eq_preset_treble_boost = 高音ブースト
eq_preset_vocal_boost = ボーカルブースト
eq_preset_loudness = Loudness
eq_preset_custom = カスタム
move_to_folder = フォルダーに移動
no_folders_yet = フォルダーがまだありません。
folder_name = フォルダー名...
new_folder = 新しいフォルダー
remove_from_folder = フォルダーから削除
back_behavior = 戻るボタンの動作
back_behavior_rewind = 巻き戻し → 前へ
back_behavior_always_prev = 常に前へ


channel_mode = チャンネルモード
channel_mode_stereo = ステレオ
channel_mode_mono = モノラル
channel_mode_left_only = 左のみ
channel_mode_right_only = 右のみ
channel_mode_swap_left_right = 左右を入れ替え
device_change_behavior = 出力デバイス変更時
device_change_resume = 再生を続ける
device_change_pause = 一時停止
sample_rate_mode = 出力サンプルレート
sample_rate_mode_system = システムに従う
sample_rate_mode_source = トラックに合わせる

crossfade = クロスフェード
crossfade_off = オフ

continue_listening = 続きを聴く
music = 音楽
made_for_you = あなたのために
recently_added = 最近追加
customize_home = ホームをカスタマイズ
done_editing = 完了
reset_home = リセット
hide_section = 非表示
show_section = 表示
move_up = 上へ
move_down = 下へ
home_section_hero = ヒーロー
home_section_continue_listening = 続きを聴く
home_section_listen_now = 今すぐ聴く
home_section_top_artists = トップアーティスト
home_section_new_releases = 新作
home_section_made_for_you = あなたのために
home_section_recently_added = 最近追加
home_section_playlists = プレイリスト
layout_cards = カード
layout_list = リスト
listen_now_layout = レイアウト
ui_style = UIスタイル
ui_normal = 通常
ui_vaxry = Vaxry
player_bar_position = プレーヤーバーの位置
position_bottom = 下
position_top = 上

metadata = メタデータ
auto_fetch_covers = アルバムカバーを自動取得
prefer_local_lyrics = ローカル歌詞を優先（.lrc / 埋め込み、オンラインフォールバックなし）
enable_musixmatch_lyrics = Musixmatchを歌詞の予備ソースとして使用
lyrics_depth_blur = アクティブな行からの距離に応じて歌詞の行をぼかす
lyrics_depth_blur_strength = 歌詞のぼかしの強さ
lyrics_offset = 歌詞のオフセット
lyrics_offset_auto = 歌詞のオフセットを自動で決定
cover_fetch_strategy = カバー取得戦略
musicbrainz_first = MusicBrainz優先
lastfm_first = Last.fm優先
musicbrainz_only = MusicBrainzのみ
lastfm_only = Last.fmのみ
remove_cover = カバーを削除

# Radio Page
radio_filter_stations = ステーションをフィルタリング…
radio_search_stations = ステーションを検索…
radio_selected = セレクション
radio_top_stations = 人気のステーション
radio_search_results = 検索結果
radio_powered_by = radio-browser.info 提供
radio_loading_stations = ステーションを読み込み中…
radio_search_failed = radio-browser.info に接続できませんでした
radio_subtitle = ライブインターネットラジオストリームを聴く
radio_no_stations_match = 検索に一致するステーションはありません
radio_station_col = ステーション
radio_description_col = 説明
radio_streams_col = ストリーム
radio_listen_moe_desc = アニメとK-Pop音楽を24時間年中無休で。
radio_j1_desc = 日本の最新チャートヒットと名曲。
radio_doujinstyle_desc = 同人音楽、ビデオゲームのサウンドトラックなど。
radio_vocaloid_desc = 日本のボーカロイドヒット曲。何でも再生します！
radio_asiadreamradio_desc = ジェイポップミュージックの中心 - ジェイポップ、ジェイロック、ジェイヒップホップ、ジェイジャズ、クラシック。
radio_live_stream = ライブストリーム

auto_check_updates = アップデートを自動確認
minimize_to_tray = システムトレイに最小化
update_available = アップデートがあります
update_banner_message = Kopuz { $version } が利用可能です。
view_release = リリースを見る

volume_scroll_step = 音量スクロールステップ

# Radio Registries
radio_registry_url_placeholder = index.json への URL またはローカルパス
add_radio_registry = ラジオレジストリを追加
radio_default_registry = デフォルトレジストリ


radio_registry_empty_path = レジストリパスを空にすることはできません
radio_registry_exists = レジストリは既に存在します
radio_registry_import_failed = レジストリのインポートに失敗しました: { $error }
radio_registry_enable_failed = レジストリの有効化に失敗しました: { $error }
radio_registries_active = { $enabled_count }/{ $total } アクティブ
radio_registries_empty = レジストリが設定されていません
offline_downloads = オフラインダウンロード
download_quality = ダウンロード品質
saving = 保存中...
discover_more_loading = さらに読み込み中…
discover_failed = 発見を読み込めません: { $error }
discover_requires_ytmusic = 発見は YouTube Music がアクティブなサーバーの場合にのみ利用できます。
discover_show_all = すべて表示
yt_anon_playlists = ライブラリのプレイリストを表示するには YouTube Music にログインしてください。
yt_anon_favorites = 高評価した曲を表示するには YouTube Music にログインしてください。

# Metadata Viewer / Editor
view_metadata = メタデータを表示
edit_metadata = メタデータを編集
edit = 編集
track_number = トラック番号
disc_number = ディスク番号
duration = 再生時間
sample_rate = サンプルレート
bitrate = ビットレート
musicbrainz_release = MusicBrainz リリース
musicbrainz_recording = MusicBrainz レコーディング
musicbrainz_track = MusicBrainz トラック
path = パス
add_photo = 写真を追加
change_photo = 写真を変更
remove_photo = 写真を削除
metadata_edit_warning = 空欄にするとそのタグは削除されます。ファイルに直接書き込まれ、元に戻せません。

sort_by = 並べ替え
sort_then = 次に
sort_none = 並べ替えなし
sort_add_criterion = 条件を追加
sort_remove = 削除
sort_ascending = 昇順
sort_descending = 降順
sort_field_title = タイトル
sort_field_artist = アーティスト
sort_field_year = 年
sort_field_genre = ジャンル
sort_field_album = アルバム
sort_field_duration = 再生時間
sort_field_date_added = 追加日
sort_field_name = 名前
sort_field_tracks = トラック
sort_field_albums = アルバム
view_grid = グリッド表示
view_list = リスト表示
spotify_browser = Spotify再生ブラウザ
spotify_browser_auto = 自動
spotify_connect_device = Spotify の再生デバイス
spotify_connect_device_other = 他のデバイス
spotify_connect_device_this = このアプリ
spotify_play_on = 再生デバイス
spotify_this_app = kopuz（このアプリ）
settings_managed_notice = 一部の設定はシステム構成（Nix/hjem）で管理されており、ここでは変更できません。
setting_managed_by_system = システム構成で管理されています
