language-name = sitelen pona
is_rtl = false

settings = 󱤿󱤎
settings_layout = nasin tawa pi lipu nasin
settings_layout_cd = CD
settings_layout_topbar = nasin sewi
theme_editor = 󱥄󱤆󱤉󱤮
appearance = 󱤮
language = 󱥬
library = 󱥓󱤕
playlists = 󱤟󱤕
album = 󱤟󱥍󱦗󱤕󱥖󱦘
artist = 󱤑󱥉
home = 󱥭
search = 󱥄󱤃
favorites = 󱥅
radio = ilo kalama
discover = lukin sin
logs = 󱥠󱥡
activity = 󱥉
general = 󱤄
local_libraries = kulupu kalama lon ilo
active_local_library = pali
switch_to_local_library = ante
add_local_library = o pana e kulupu kalama lon ilo
local_library_name = nimi kulupu
local_library_name_required = nimi kulupu li wile
local_library_folder_required = o alasa e poki lipu wan anu mute
service = 󱤿󱥌󱦝{ $name }
connected = ● 󱤬
disconnected = ● 󱤬󱤂
connecting = ● 󱤥󱤬
reconnect = 󱥄󱤬󱥝
remove = 󱥄󱥶
add = 󱥄󱥌
add_folder = 󱥄󱥌 󱤉 󱤕
no_music_folders = 󱤕 󱤧 󱤬 󱤂
remote_music_folders = poki kalama
parent_folder = o tawa sewi
use_this_folder = o namako e poki ni
no_subfolders = poki lili li ala
loading_folders = mi kama jo e poki...
folder_browse_failed = mi ken ala lukin e poki: { $error }
add_server = 󱥄󱥸︀󱤉󱥌
connect_to_lastfm = o lon tawa Last.fm
connect_to_librefm = o lon tawa Libre.fm
lastfm_connected = Last.fm li lon
librefm_connected = Libre.fm li lon
server_details = 󱥡󱥌
server_name = 󱥂󱥌
server_url = 󱤿URL 󱥌
choose_service = 󱥄󱥳󱤉󱥷󱥍󱦗󱤿󱥌󱦘
cancel = 󱤂
login = 󱥄󱤖
logging_in = 󱤖󱦜󱦜󱦜
login_to_service = 󱥄󱤖󱥩{ $service }
username = 󱥂
password = 󱤥
player_settings = 󱤿󱥍󱤎󱤤󱤕
discord_presence = 󱤬󱥍󱤎󱦐󱥛󱦜󱤜󱦝󱦑
connectivity = 󱥩󱤬󱥭
discord_presence_paused = 󱥄󱤮󱤉 󱤬󱥍󱤎󱦐󱥛 󱥫
discord_presence_source = 󱥄󱤮󱤉 󱤎 󱤬 󱤬󱥍󱤎󱦐󱥛󱦜󱤜󱦝󱦑
reduce_animations = 󱥄󱤨󱤉󱥩
fullscreen_use_player_bar = Use player bar as fullscreen controls
cover_art_background = Cover Art Background
cover_art_darkening = Cover Art Darkening
cover_art_blur = Cover Art Blur
custom_background = Custom Background
choose_image = Choose Image
custom_font = Custom Font
choose_font = Choose Font
live_theme = Matugen / Pywal (live)
live_theme_file = Palette File
choose_palette = Choose Palette
jump_to_current_song = Jump to current song
hide_side_panel = Hide side panel
show_side_panel = Show side panel
show_source_toggle = 󱥄󱤮󱤉󱤎󱤆
show_row_images = Show Row Images
titlebar_mode = 󱤤󱤎
titlebar_custom = 󱤤󱥎󱤎
titlebar_system = 󱤤󱥎󱤎󱥛
titlebar_off = 󱤤󱤂
volume = 󱥣󱤕
local = 󱥧󱤎󱥁
source = Source
sources = Sources
manage_sources = Manage sources
source_on_this_device = On this device
server = 󱥧󱥌
youtube_downloads = 󱤖󱤓
youtube_download_title = 󱤖󱤓󱥧 YouTube
youtube_download_subtitle = 󱥄󱤃󱤉󱥂󱤬 YouTube Music, 󱤇󱥄󱥌󱤉󱤩󱥂. Kopuz 󱤧󱤖󱤓󱤉󱥆
youtube_download_options = 󱤿󱤆
youtube_download_search_placeholder = 󱥂󱤕, 󱤑, 󱥓, 󱤇󱤩 nimi…
youtube_download_search = 󱥄󱤃
youtube_download_download_link = 󱥄󱤖󱤓󱤉󱤩󱥂
youtube_download_resolving_link = 󱤴󱤮󱤉󱤩 nimi…
youtube_download_link_detected = 󱤩󱥂󱤧󱤬. 󱥁󱤧󱤖󱤓󱤉󱤕󱥁󱥨
youtube_download_error_link = 󱤴󱤘󱤂󱤮󱤉󱤩󱥂: { $error }
youtube_download_searching = 󱤴 alasa…
youtube_download_results = 󱤖󱥧󱤃
youtube_download_download = 󱥄󱤖󱤓
youtube_download_output_dir_placeholder = 󱥓󱥍󱤖󱤓 (poki 󱤕󱤧󱤿 open)
youtube_download_pick_folder = 󱥄󱥷󱤉󱥓
youtube_download_clear_history = 󱥄󱥶󱤉󱥡󱥐
youtube_download_empty_state = 󱥄󱤃󱤉󱤕, 󱤇󱥄󱥌󱤉󱤩󱥂 YouTube
youtube_download_no_results = 󱤕󱤂󱤧󱤬
youtube_download_jobs = 󱤖󱤓
youtube_download_file_options = 󱤿󱤆󱥍󱤪󱤕
youtube_download_embed_metadata = 󱥄󱥌󱤉󱥡󱤬󱤏
youtube_download_embed_metadata_desc = 󱥂, 󱤑, 󱥓, 󱤊󱤩󱥂󱤱󱤧󱤈󱤬󱤏󱥍󱤪󱤕
youtube_download_embed_thumbnail = 󱥄󱥌󱤉󱥠󱥓󱤬󱤏
youtube_download_embed_thumbnail_desc = 󱥞󱤘󱤡󱥆󱤧󱤬󱤏. 󱥞󱤘󱤂󱤡󱤪 JPG 󱤧󱤬󱥒
youtube_download_write_thumbnail = 󱥄󱤈󱤉󱥠󱥓󱤬󱤪󱤆
youtube_download_write_thumbnail_desc = 󱤪 JPG 󱤧󱤖󱤬󱥒󱥍󱤪󱤕
youtube_download_organize_album = 󱥄󱤿󱤉󱥆󱤙󱥓
youtube_download_organize_album_desc = 󱤪󱤖󱤓󱤧󱤬󱥓󱥍󱥂󱥓
youtube_download_overwrite = 󱥄󱤆󱤉󱤪󱥍󱤬󱥐
youtube_download_overwrite_desc = 󱤪󱥍󱥂󱥖󱤧󱤖󱤆
youtube_download_ffmpeg_hint = 󱤖󱤓󱥍󱤕󱥇󱤧󱥷󱤂󱤉󱤎󱤆. 󱤎 ffmpeg 󱤧󱥷󱥨󱥩󱤆󱤕, 󱥩󱥡, 󱤇󱥩󱥠󱥓󱤬󱤏.
youtube_download_format_original = 󱤕󱥇
youtube_download_format_mp3 = MP3
youtube_download_format_flac = FLAC
youtube_download_format_opus = OPUS
youtube_download_format_wav = WAV
youtube_download_status_resolving = 󱤴󱤃󱤉󱤿󱤕 YouTube…
youtube_download_status_downloading_eta = { $percent }%  { $speed }  󱥫󱤈 { $eta }
youtube_download_status_downloading = { $percent }%
youtube_download_status_processing = 󱤴 pali…
youtube_download_status_completed = 󱥐
youtube_download_status_waiting = 󱤴 awen…
youtube_download_error_search = 󱤃󱤧󱥈: { $error }
youtube_download_error_ffmpeg_required = 󱤿󱥁󱤧󱥷󱤉󱤎 ffmpeg. 󱥄󱤖󱤓󱤉󱤎 ffmpeg, 󱤇󱥄󱥷󱤉󱤕󱥇󱤉󱥶󱥍󱥌󱤏.
youtube_download_error_duplicate_active = 󱤕󱥁󱤧󱤖󱤓󱤬󱥫󱥁.
youtube_download_error_output_not_directory = 󱤿󱥁󱤧󱥓󱤂: { $path }
youtube_download_error_output_prepare = 󱤴󱤘󱤂󱥉󱤉󱥓: { $error }
youtube_download_error_output_not_writable = 󱤴󱤘󱤂󱥌󱤬󱥓󱥁: { $path }

# UI Actions & Buttons
add_to_favorites = 󱥄󱥌󱥩󱥅
remove_from_favorites = 󱥄󱥶󱥧󱥅
add_to_queue = 󱥄󱥌󱥩󱤾󱤕
play_next = 󱥄󱤕󱤬󱥒
add_to_playlist = 󱥄󱥌󱥩󱤟󱤕
remove_from_playlist = 󱥄󱥶󱥧󱤟󱤕
start_radio = 󱥄󱥇󱤉󱤾󱤕󱥝
radio_starting = nasin kalama li open…
radio_empty = mi lon ala e kalama sama
radio_failed = mi ken ala open e nasin kalama
share_musicbrainz = 󱥄 (MusicBrainz)
share_link_copied = linja MusicBrainz li kama jo
delete = 󱥄󱥶
delete_song = 󱥄󱥶󱤉󱤕󱤻
delete_album = 󱥄󱥶󱤉󱤟󱥍󱤕󱥖
add_all_to_queue = 󱥄󱥌󱤉󱤄󱥩󱤾󱤕
add_all_to_playlist = 󱥄󱥌󱤉󱤄󱥩󱤟󱤕
select_all = 󱥄 󱤭 󱤉 󱤄
select_all_tracks = 󱥄 󱤭 󱤉 󱤕 󱤻 󱤄
remove_from_cache = 󱥄󱥶󱥧󱥡󱤨
create = 󱥄󱥉
save = 󱥄󱤖󱤓
enabled = 󱤬
disabled = 󱤬󱤂
play = o kalama
shuffle = o nasa e nasin kalama

# Navigation & Headers
tracks = 󱤕󱤻
albums = 󱤟󱥍󱦗󱤕󱥖󱦘
artists = 󱤑󱥉
all_albums = 󱤄󱥍󱤟󱥍󱦗󱤕󱥖󱦘
your_library = 󱥓󱤕󱥞
listening_logs = 󱥠󱥡󱥍󱦗󱤕󱤠󱦘
browse_genres = 󱥄󱤮󱤉󱤞
top_artists = 󱤑󱥉󱥔󱤀
new_releases = 󱤕󱥝

# Navigation Buttons
up_next = 󱥒
lyrics = 󱥠

# Player/Media
loading_lyrics = 󱤴󱤖󱤓󱤉󱥠󱦜󱦜󱦜
lyrics_not_found = 󱥠󱤧󱤬󱤂
instrumental_break = kalama musi taso
no_previous_songs = 󱤕󱥐󱤧󱤬󱤂
playlist_track_count = 󱤕󱤻 { $count }
music_playlist_count = 󱤕󱤻 • { $count }
showcase_song_count = 󱤕󱤻 { $count }

# Modal Titles
add_playlist = 󱥄󱥌󱤉󱤟󱤕
create_new_playlist = 󱥄󱥉󱤉󱤟󱤕󱥝
playlist_name_placeholder = 󱥂󱥍󱦗󱤟󱤕󱦘
playlist_name_input = 󱥂󱥍󱦗󱤟󱤕󱦘
add_media_server = 󱥄󱥸︀󱤉󱥌
media_server = 󱥌
media_servers = 󱥌 󱤴
saved_servers = 󱥌 󱥩
switch_to_server = 󱥄󱤆
active_server = 󱥌 󱤡
no_saved_servers = 󱥌󱤍 󱥩
jellyfin = Jellyfin
subsonic = Subsonic
custom_manual = Custom (manual API)
server_name_optional = 󱥂󱥌 (󱤘)
server_url_placeholder = http://localhost:8096
nextcloud_url_placeholder = https://cloud.example.com
change = 󱥄󱤆
heart_track_to_add = 󱥄󱤭󱤉󱥀󱥅󱤬󱤠󱤕󱤡󱤕󱤧󱤖󱥩󱥁
heart_track_to_add_server = 󱥄󱤭󱤉󱥀󱥅󱤬󱤠󱤕󱤡󱤕󱤧󱤖󱥩󱥁󱦜󱥹󱤡󱤴󱥌󱤉󱥡󱥁󱥩󱥌

# Search & Placeholders
search_placeholder = 󱥄󱤃󱤉󱤑󱥉, 󱤉󱤟󱥍󱤕󱥖, 󱤉󱤕󱦜󱦜󱦜
quick_search_placeholder = 󱥄󱤃, 󱥄󱤕󱤻󱦜󱦜󱦜
no_results_found = "{ $query }"󱤧󱤬󱤂
listenbrainz_token_placeholder = 󱥄󱥌󱤉"token ListenBrainz"
lastfm_api_key_placeholder = o pana e Last.fm API key sina
lastfm_api_secret_placeholder = o pana e Last.fm API secret sina

# Empty States
album_not_found = 󱤟󱥍󱦗󱤕󱥖󱦘󱤧󱤬󱤂
playlist_not_found = 󱤟󱤕󱤧󱤬󱤂
no_playlists_found = 󱤟󱤕󱤧󱤬󱤂
no_playlists_yet = 󱤟󱤕󱤧󱤬󱤂󱦜󱥄󱥉
add_music_to_get_started = 󱥷󱥇󱤡󱥄󱥌󱤉󱤕

# Data Labels
title = 󱥂
time = 󱥫
genre = 󱤞
plays = 󱤼󱤠
track_count = 󱤕󱤻 { $count }
track_count_singular = 󱤕󱤻󱥳
songs = 󱤕󱤻
min = 󱥫

# Error Messages
invalid_server_url = 󱤿URL 󱤧󱤍
server_name_required = 󱥂󱥌 󱤧󱥢
username_and_password_required = 󱥞󱥄󱥌󱤉󱥂󱤉󱤥
login_failed = 󱤖󱤧󱤍󱦝{ $error }
error_server_not_configured = 󱥌󱤧󱤍󱤇󱤓󱤂󱤉󱤥
error_fetch_songs = 󱤴󱤘󱤂󱤖󱤓󱤉󱤕󱥩󱤟{ $album_id }󱦝{ $error }
unsupported_provider = 󱥌{ $service }󱤧󱥔　󱥨󱤪󱥁󱤧󱤬󱤂󱥩󱥆
unsupported_provider_desc = 󱤃󱥍󱥌{ $service }󱤧󱤬󱤂󱤧󱤖󱤬󱥫󱥒

# Additional Keys
no_genres_found = 󱤞󱤧󱤬󱤂
jump_back_in = 󱥄󱤠󱥝
by_artist = 󱥧
by_artist_full = 󱥧󱤑{ $artist }
start_listening = 󱥄󱥇󱤠
listen_now = 󱥄󱤠󱤬󱥫󱥁
no_albums_found = 󱤟󱥍󱦗󱤕󱥖󱦘󱤧󱤬󱤂
no_favorites = 󱥅󱤧󱤬󱤂
no_tracks_found = 󱤕󱤧󱤬󱤂
unknown_artist = 󱤑󱤂󱥉
unknown_album = 󱤟󱤂󱥍󱦗󱤕󱥖󱦘
unknown_title = 󱥂󱤂
unknown_genre = 󱤞󱤂
most_played_local_tracks = 󱤕󱥍󱦗󱤠󱤼󱥞󱦘
no_tracks_in_library = 󱤕󱤧󱤬󱤂
no_songs_here = 󱤕󱤧󱤬󱤂󱥁
syncing_with_server = 󱤴󱥬󱥩󱥌󱦜󱦜󱦜
yt_syncing_progress = mi pana e kalama musi { $count }
yt_synced_total = kalama musi pona { $count } li lon
refresh = sin
most_played_tracks = 󱤕󱥍󱦗󱤠󱤼󱥞󱦘
no_more_songs = 󱤕󱥒󱤧󱥐
server_playlist = 󱤟󱤕󱥍󱦗󱥧󱥌󱦘
folder_playlists = 󱤟󱤕 󱥍󱦗󱤕 󱤄󱦘
shuffle_on = 󱥄󱤾󱤉󱤿󱤕󱦝󱤬
shuffle_off = 󱥄󱤾󱤉󱤿󱤕󱦝󱤬󱤂
repeat_off = 󱥄󱥝󱦝󱤂
repeat_queue = 󱥄󱥝󱦝󱤄
repeat_track = 󱥄󱥝󱦝󱥳
rescan_library = 󱥄󱤮󱥝󱤉󱥓󱤕
refresh_music_library = 󱥄󱤮󱥝󱤉󱥓󱤕
listenbrainz = ListenBrainz
lastfm = Last.fm
librefm = Libre.fm
enable_tracing = 󱥄󱥇󱤉󱤮
tracing_warning = 󱥄󱥇󱥝󱤉󱤎
export_logs = 󱥄󱥌󱤉󱥠󱥡
open_logs_folder = 󱥄󱥇󱤉󱥓󱥠󱥡
album_art_gradient = Album art gradient
default_theme = Default
gruvbox_material = Gruvbox Material
gruvbox_classic = Gruvbox Classic
gruvbox_dark_soft = Gruvbox Dark Soft
dracula = Dracula
nord = Nord
catppuccin_mocha = Catppuccin Mocha
ef_night = Ef Night
ayu_dark = Ayu Dark
ayu_mirage = Ayu Mirage
vague = Vague
one_dark_pro = One Dark Pro
osmium = Osmium
kanagawa_dragon = Kanagawa Dragon
everforest = Everforest
rosepine = Rosé Pine
default_light = Default Light
amoled_black = AMOLED Black
catppuccin_latte = Catppuccin Latte
rosepine_dawn = Rosé Pine Dawn
everforest_light = Everforest Light
ayu_light = Ayu Light
one_light = One Light
gruvbox_light_soft = Gruvbox Light Soft

# Theme Editor
new_theme = 󱥄󱥉󱤉󱤮󱥝
theme_name = Theme Name
my_custom_theme = My Custom Theme
theme_group_dynamic = Dynamic
theme_group_dark = Dark
theme_group_light = Light
theme_group_custom = Custom
colors = Colors
preview = Preview
save_theme = Save Theme
track_title = Track Title

# Color Labels
bg = Background
raised = Raised Surface
surface = Surface
text = Text
text-muted = Text Muted
accent = Accent
accent-soft = Accent Soft
accent-alt = Accent Alt
accent-deep = Accent Deep
highlight = Highlight
highlight-dark = Highlight Dark
progress = Progress
danger = Danger


featured_album = 󱥯 󱤶 󱥬
minimize = 󱥬 󱤴
maximize = 󱥬 󱤿
close = 󱥔
mini_player = ilo lili
restore_full_player = ilo suli
delete_playlist = 󱥄󱥶 󱤉 󱤟󱤕
delete_folder = 󱥄󱥶 󱤉 󱤕 󱤄
rename_playlist = 󱥄󱥂󱥍 󱤉 󱤟󱤕
rename_folder = 󱥄󱥂󱥍 󱤉 󱤕 󱤄
equalizer = 󱤉󱤺 󱥟
eq_preset = 󱤄 󱤂
eq_preamp = 󱤄 󱤟 󱥣
eq_preamp_desc = 󱤓 󱤧 󱥞 󱤉 EQ
eq_graph_hint = 󱤄 󱥄󱤮 󱤉 󱤄 li kama custom.
eq_preset_flat = 󱤳
eq_preset_bass_boost = 󱥄 󱥣 󱤉 󱤧 󱤓
eq_preset_treble_boost = 󱥄 󱥣 󱤉 󱤪 󱤓
eq_preset_vocal_boost = 󱥄 󱥣 󱤉 󱤄 󱤓
eq_preset_loudness = Loudness
eq_preset_custom = custom
move_to_folder = 󱥄󱤂 󱤉 󱤕
no_folders_yet = 󱤕 󱤧 󱤬 󱤂.
folder_name = 󱥂 󱤉 󱤕...
new_folder = 󱤕 󱤿
remove_from_folder = 󱥄󱥶 󱤉 󱤕
back_behavior = 󱤍 󱤉 󱤏 monsi
back_behavior_rewind = 󱥄 󱤄 󱤂 → 󱤗
back_behavior_always_prev = 󱤗 󱤄
folder_playlist = 󱤕

channel_mode = 󱥔 󱤴
channel_mode_stereo = 󱤴 󱥩
channel_mode_mono = 󱤴 󱥯
channel_mode_left_only = 󱥭 󱤿 󱥔
channel_mode_right_only = 󱥭 󱥬 󱥔
channel_mode_swap_left_right = 󱤴 󱥭
device_change_behavior = On output device change
device_change_resume = Keep playing
device_change_pause = Pause
sample_rate_mode = nasin nanpa kalama
sample_rate_mode_system = sama ilo
sample_rate_mode_source = sama kalama

crossfade = 󱥭 󱤴
crossfade_off = 󱤄

continue_listening = Continue Listening
music = 󱤕󱤻
made_for_you = Made For You
recently_added = Recently Added
customize_home = Customize Home
done_editing = Done
reset_home = Reset
hide_section = Hide
show_section = Show
move_up = Move up
move_down = Move down
home_section_hero = Hero
home_section_continue_listening = Continue Listening
home_section_listen_now = Listen Now
home_section_top_artists = Top Artists
home_section_new_releases = New Releases
home_section_made_for_you = Made For You
home_section_recently_added = Recently Added
home_section_playlists = Playlists
layout_cards = Cards
layout_list = List
listen_now_layout = Layout
ui_style = UI style
ui_normal = Normal
ui_vaxry = Vaxry
player_bar_position = Player bar position
position_bottom = Bottom
position_top = Top

metadata = Metadata
auto_fetch_covers = Auto-fetch album covers
prefer_local_lyrics = Prefer local lyrics (.lrc / embedded, no online fallback)
enable_musixmatch_lyrics = Use Musixmatch lyrics fallback
lyrics_depth_blur = Blur lyric lines by distance from the active line
lyrics_depth_blur_strength = wawa pi lukin ike nimi
lyrics_offset = Lyrics offset
lyrics_offset_auto = Determine lyrics offset automatically
cover_fetch_strategy = Cover fetch strategy
musicbrainz_first = MusicBrainz first
lastfm_first = Last.fm first
musicbrainz_only = MusicBrainz only
lastfm_only = Last.fm only
remove_cover = Remove cover

# Radio Page
radio_filter_stations = o lukin e ilo kalama…
radio_search_stations = o alasa e ilo kalama…
radio_selected = ijo pi wile sina
radio_top_stations = ilo kalama pi jan mute
radio_search_results = ijo alasa
radio_powered_by = ni li tan ilo radio-browser.info
radio_loading_stations = ilo kalama li kama…
radio_search_failed = mi ken ala toki tawa ilo radio-browser.info
radio_subtitle = o kute e ilo kalama linja
radio_no_stations_match = ilo kalama ala li sama wile sina
radio_station_col = ilo kalama
radio_description_col = sona
radio_streams_col = linja kalama
radio_listen_moe_desc = kalama musi pi ma Nijon en ma Kanto.
radio_j1_desc = kalama musi suli pi ma Nijon.
radio_doujinstyle_desc = kalama musi pi musi sitelen.
radio_vocaloid_desc = kalama musi pi ilo kalama.
radio_asiadreamradio_desc = insa pi kalama musi J-Pop - J-Pop, J-Rock, J-HipHop, J-Jazz, kalama pi tenpo pini.
radio_live_stream = linja kalama tenpo ni

auto_check_updates = lukin pi sin namako lon tenpo ale
minimize_to_tray = lili tawa poki ilo
update_available = sin namako li lon
update_banner_message = Kopuz { $version } li lon.
view_release = o lukin e lipu pi sin namako

volume_scroll_step = nasin tawa kalama

# Radio Registries
radio_registry_url_placeholder = URL anu nasin tawa lipu index.json
add_radio_registry = o pana e lipu kalama ilo
radio_default_registry = lipu pi nanpa wan


radio_registry_empty_path = nasin pi lipu ijo li ken ala ala
radio_registry_exists = lipu ijo li lon pini
radio_registry_import_failed = kama jo e lipu ijo li pakala: { $error }
radio_registry_enable_failed = open e lipu ijo li pakala: { $error }
radio_registries_active = lipu ijo { $enabled_count } tan { $total } li pali
radio_registries_empty = lipu ijo ala li lon
offline_downloads = ijo tawa ilo
download_quality = pona pi ijo tawa ilo
saving = awen...
discover_more_loading = mi alasa e ijo sin…
discover_failed = pakala — mi ken ala alasa: { $error }
discover_requires_ytmusic = sona sin li lon taso lon tenpo pi YouTube Music.
discover_show_all = lukin e ale
yt_anon_playlists = o kama lon YouTube Music la sina ken lukin e kulupu kalama sina.
yt_anon_favorites = o kama lon YouTube Music la sina ken lukin e kalama pi olin sina.

# Metadata Viewer / Editor
view_metadata = o lukin e ijo nanpa
edit_metadata = o ante e ijo nanpa
edit = o ante
track_number = nanpa pi kalama musi
disc_number = nanpa pi sike kalama
duration = tenpo
sample_rate = nanpa pi kipisi kalama
bitrate = nanpa pi suli kalama
musicbrainz_release = kulupu MusicBrainz
musicbrainz_recording = kalama MusicBrainz
musicbrainz_track = kalama musi MusicBrainz
path = nasin lipu
add_photo = o pana e sitelen
change_photo = o ante e sitelen
remove_photo = o weka e sitelen
metadata_edit_warning = nimi ala li weka e nimi nanpa. ni li ante e lipu. sina ken ala ante e ni.

sort_by = o leti kepeken
sort_then = kin la
sort_none = leti ala
sort_add_criterion = o namako e nasin
sort_remove = o weka
sort_ascending = tan lili tawa suli
sort_descending = tan suli tawa lili
sort_field_title = nimi
sort_field_artist = jan pali
sort_field_year = tenpo sike
sort_field_genre = kule
sort_field_album = kulupu pi kalama sama
sort_field_duration = tenpo
sort_field_date_added = tenpo kama
sort_field_name = nimi
sort_field_tracks = kalama musi
sort_field_albums = kulupu pi kalama sama
view_grid = lukin leko
view_list = lukin linja
spotify_browser = ilo lukin pi kalama Spotify
spotify_browser_auto = ilo li kama pona sama
spotify_connect_device = ilo pi kalama Spotify
spotify_connect_device_other = ilo ante
spotify_connect_device_this = ilo ni
spotify_play_on = o kalama lon
spotify_this_app = kopuz (ilo ni)
settings_managed_notice = nasin ilo mute li lawa tan nasin pi ilo suli (Nix/hjem) la sina ken ala ante e ona lon ni.
setting_managed_by_system = nasin pi ilo suli li lawa e ni
