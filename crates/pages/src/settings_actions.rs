use ::server::plugin::{AuthPrompt, AuthValues};
...
pub fn delete_saved(mut config: Signal<AppConfig>, id: String) {
    ...
    match service {
        ...
        Some(MusicService::Plugin) => {
            // Unload it and forget it. The plugin's own data directory is left
            // alone: it is not Kopuz's to delete.
            if let Some(plugin_id) = saved.and_then(|s| s.plugin_id) {
                spawn(async move {
                    ::server::plugin::auth_cancel(&plugin_id).await;
                    ::server::registry().disconnect(&plugin_id).await;
                });
            }
        }
        _ => {}
...
pub fn plugin_auth_begin(
    ...
    spawn(
        async move {
            let prompt = ::server::plugin::auth_begin(&plugin_id).await;
...
pub fn plugin_auth_submit(
    ...
    spawn(
        async move {
            let prompt = ::server::plugin::auth_submit(&plugin_id, values).await;
            finish_step(auth_state, error, plugin_id, plugin_name, prompt);
        }
...
pub fn plugin_auth_cancel(mut auth_state: Signal<Option<PluginAuthState>>) {
    ...
    spawn(async move {
        ::server::plugin::auth_cancel(&state.plugin_id).await;
    });
}
...
fn finish_step(
    ...
    plugin_id: String,
    plugin_name: String,
    prompt: Result<AuthPrompt, String>,
) {
    ...
    // A runtime failure is shown the same way the plugin's own `Failed` would
    // be: the user cannot act on the distinction.
