...
impl Session {
    ...
    // kopuz: upstream calls `exit(1)` here, taking the whole app down when
    // the account is not premium. A free account keeps its library, playlists
    // and search; only the audio key request is refused, and the player
    // reports that per track.
    fn check_catalogue(attributes: &UserAttributes) {
        if let Some(account_type) = attributes.get("type") {
            if account_type != "premium" {
                warn!("Spotify {account_type:?} account: playback needs premium");
            }
        }
    }
